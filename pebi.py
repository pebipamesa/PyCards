...
class PyCards:
    ...
    def show_main(self):
        ...
        def show_main_component():
            ...
            card_container = tk.Frame(self.main_component, bg="white")
            card_container.grid(row=1, column=0, sticky="nsew")



            flashcards = get_flashcards(cursor, userInfo["id"])
            print("len", len(flashcards), len(flashcards)//4, len(flashcards)%4, len(flashcards)/4)
            if len(flashcards) != 0:
                limit = len(flashcards)/4 if len(flashcards) % 4 == 0 else (len(flashcards) // 4) + 1
                print("limit", limit)
                for i in range(int(limit)):
                    column = 0
                    range_limit = (i+1)*4 if (i+1)*4 < len(flashcards) else len(flashcards)
                    for j in range(i*4, range_limit):
                        column += 1
                        print("j", j)
            ...
            AddButton.grid(row=(len(flashcards)//4) + 1, column=(len(flashcards) % 4) + 1, sticky="new", padx=(0, 30), pady=(20, 0))
